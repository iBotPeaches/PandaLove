...
class Game
{
    ...
    public static function buildRoundArray(Match $match)
    {
        ...
        foreach ($match->events as $event) {
            if ($event->event_name == EventName::RoundStart) {
            ...
            } elseif ($event->event_name == EventName::RoundEnd) {
                if (!$match->isTeamGame) {
                    $numPlayers = 0;
                    foreach ($team_label as $team) {
                        if (!$team['dnf']) {
                ...
                $zombies = [];
                $deathCount = 1;
            } elseif ($event->event_name == EventName::WeaponPickup) {
            ...
            } elseif ($event->event_name == EventName::Death) {
                $killsObtained = true;
        ...
        foreach ($combined as $time_key => &$time) {
            foreach ($time as $user_id => &$events) {
                ...
                foreach ($events as $key => &$event) {
                    ...
                    } elseif ($event->event_name == EventName::Medal) {
                    } elseif ($event->event_name == EventName::Impulse) {
    ...
    private static function get($player, $key, $formatted = false)
    {
        ...
        } elseif (method_exists($player, $key)) {
            return $player->$key();
        } else {
            if ($formatted) {
                return $player->$key;
